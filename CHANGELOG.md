...
### Changed
- **PC 访客端布局重构**：原两栏 "聊天+介绍" 改为工作台同款结构 — 顶部红色栏 + 中间聊天区（会话头 + 消息流 + 工具栏 + 输入）+ 右侧 `360px` 平台介绍面板。与客服工作台视觉统一，去掉了不存在的"会话切换栏"。
- **移动端输入栏重设**：改为 `[+] [😊] [输入框] [发送]` 单行布局；`+` 展开 4 宫格附件面板（图片/视频/文件/订单），😊 展开 8 列表情面板；解决原本按钮与输入框挤在一起的问题。
