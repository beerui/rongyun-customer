...
<template>
  <Drawer :open="props.open" title="快捷话术" :width="400" @close="emit('close')">
    <div class="p-4 space-y-2">
      <div v-for="(t, i) in quickReplies" :key="i" class="rounded-md border border-line-light p-3 hover:border-brand-500">
        ...
        <div class="flex items-center justify-end gap-2">
          ...
          <button class="h-7 px-3 rounded bg-brand-500 hover:bg-brand-600 text-white text-[11px]" @click="emit('send', t)">立即发送</button>
        </div>
      </div>
    </div>
  </Drawer>
