...
const peerOnline = computed(() => {
  const now = Date.now()
  return im.messages.some((m) => m.senderId !== auth.userId && m.senderId !== 'me' && now - m.sentTime < FIVE_MIN)
})

const showReconnectBanner = computed(() => !!im.currentTargetId && im.status !== 'connected')

function handleSendText(text: string) {
  im.sendTextMessage(text)
}
function handleSendImage(file: File) {
  im.sendImageFile(file)
}
function handleSendVideo(file: File) {
  im.sendVideoFile(file)
}
function handleSendFile(file: File) {
  im.sendFileMessage(file)
}
  ...
  <div class="flex flex-col h-full bg-white">
    <div v-if="title" class="flex items-center justify-between px-4 py-3 border-b border-line-light bg-white">
      <div class="min-w-0 flex items-center gap-2">
        ...
        <span v-if="peerOnline" class="inline-flex items-center gap-1 text-[10px] text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded">
    ...
    >
      ...
      <button v-if="im.status !== 'connecting'" class="text-brand-500 hover:underline" @click="im.connect(auth.rcToken).catch(() => {})">
        重新连接
      </button>
    </div>
    ...
    <template v-else>
      <div v-if="showStart" class="text-center py-3 text-[11px] text-ink-600/70 bg-bg-app">{{ startTime || '今天' }} 会话开始</div>
      <MessageList :messages="im.messages" :my-user-id="auth.userId" @retry="(id) => im.retry(id)" @recall="handleRecall" />
      <MessageInput
